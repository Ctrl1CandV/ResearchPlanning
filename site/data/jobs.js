...
window.JOBS = {
  ...
  /* 地域约束是硬前置：先筛城市，再谈岗位匹配度。
     ...
     用最强岗位密度换平台与能力，再跳向更优的岗位和城市；读研期实习仍以重庆和成都为默认。
  ...
  cityPolicy: {
    ...
    transit: {
      ...
      use: '把中转当跳板、别当终点：入职的时候就想清楚 1—2 年后跳去哪（哪个城市、哪类团队、要补什么能力）。挑团队优先看方向是不是带得走（Agent 评测、记忆、平台这类），再看离职后身价涨不涨，只看薪资最高没有意义。',
  ...
  roleFamilies: [
    ...
    { id:'backend', name:'传统后端', tier:'fallback', fit:'保底线', scope:'服务端、分布式、数据库、稳定性', reason:'这是我已有的职业资本，转 AI 不能把它丢掉', claimType:'inference' }
  ],
  teams: [
    { id:'tx-wx-knowledge', company:'腾讯', name:'WXG 企业微信·企业知识挖掘和应用', cities:['成都','北京'], cityFit:'preferred', roleFamily:'agent', targetTier:'primary', recruitmentTypes:['summerIntern','campus'], careerStages:['summer2029','campus2029'], tags:['Memory','知识图谱','图存储','分布式'], summary:'做企业知识图谱、记忆压缩、图存储和高可用后端。工程经历（图存储、后端）重合度高；与论文主线的直接关联要看主线实验是否涉及记忆，当前按工程对口投。<b>它在成都有办公点，是城市和工程经历唯一同时满足要求的团队</b>，排在第一优先。', claimType:'fact', evidenceLevel:'officialPartialJD', sourceAsOf:'2026-07-28', freshness:'historicalSample', opening:'未来实习和校招 HC 待核验；须确认成都是否放学生岗', tier:'S' },
    { id:'tx-wx-memory', company:'腾讯', name:'WXG 企业微信·记忆系统', cities:['北京','成都'], cityFit:'preferred', roleFamily:'harness', targetTier:'primary', recruitmentTypes:['campus'], careerStages:['summer2029','campus2029'], tags:['Memory','压缩','上下文'], summary:'记忆主题与条件备选方向 C 相关，工程上贴合记忆系统经验。<b>但 JD 先北京、后成都</b>：成都的岗位仍要优先争取；如果最后只有北京的 offer，按中转的定位可以去，但入职前就要想清楚 1—2 年后跳去哪（杭州、上海的同方向团队）。', claimType:'fact', evidenceLevel:'officialPartialJD', sourceAsOf:'2026-07-28', freshness:'historicalSample', opening:'待核验（成都 base 优先级未知；北京 base 按中转定位可接受）', tier:'A' },
  ...
  timeline: [
    ...
    { id:'career-2027-summer', when:'2027.07—08', title:'开源与项目二', detail:'做 GLCC 或 OSPP；主线项目按 W8 检查点推进；补 K8s 基础。代码知识图谱是第三储备，仅当启动条件满足才排期', type:'plan' },
  ...
  verification: [
    ...
    { id:'verify-repl', area:'科研', title:'主线基线的复现可行性', impact:'high', status:'open', evidence:'needsVerification', action:'核实 2606.09863 的语料与代码是否公开、tau2-bench 和 AppWorld 的环境搭建成本；90 天计划 W2 前确认，直接影响策略对比实验的可行性' },
