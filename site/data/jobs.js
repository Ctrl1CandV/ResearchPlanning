...
window.JOBS = {
  ...
  stats: {
    ...
    cities: [
      { city:'北京', count:1084, pct:84.2 }, { city:'上海', count:190, pct:14.8 },
      { city:'深圳', count:133, pct:10.3 }, { city:'新加坡', count:21, pct:1.6 },
      { city:'广州', count:14, pct:1.1 }, { city:'杭州', count:7, pct:0.5 },
      { city:'成都', count:7, pct:0.5 }, { city:'大连', count:7, pct:0.5 },
      { city:'武汉', count:3, pct:0.2 }, { city:'重庆', count:2, pct:0.2 }
    ],
    quality: [
  ...
  internshipLadder: [
    ...
    { id:'ladder-3', step:'3', when:'2028.02 起', title:'2029 届暑期实习：深圳 / 成都 / 杭州 / 上海', actions:['2027.12 开始跟踪，明确排除北京岗位','2028.02—03 主投（深圳证据最强，杭州须先补数据）','2028.04—06 面试与补录','2028.07—08 实习并争取转正'], acceptance:'在意向城市进入目标团队或获得同层级可转正机会', claimType:'forecast' }
  ],
  timeline: [
    ...
    { id:'career-2028-q1', when:'2028.01—03', title:'简历冻结与暑期实习主投', detail:'按杭州→深广沪→成都顺序投递；具体窗口属历史规律外推，2027.12 起按公司重新核验', type:'forecast', milestone:true },
  ...
  regions: [
    { city:'重庆', role:'研一驻地（不离渝）', tier:'preferred', strategy:'研一默认不离渝：用开源实习、校内产业项目和本地短期机会攒证据，把远程工作正规化。', risk:'大厂技术岗极少，腾讯云岗位偏交付；本地机会多为线索而非已核验在招。' },
    { city:'成都', role:'读研期首选跳板（1—2 小时高铁圈）', tier:'preferred', strategy:'读研期最现实的异地目标：与重庆同城化通勤，寒暑假可去而不必长期脱离学业。重点盯腾讯 WXG 企业知识/记忆方向，研二前建立联系。', risk:'当前 JD 只证明团队在成都有 base，不保证成都承接学生 HC——这是首要待核验项。' },
    { city:'杭州', role:'毕业后首选（数据缺口）', tier:'preferred', strategy:'毕业主投方向，并有本科导师人脉可用。但本轮快照未覆盖杭州雇主，须优先补抓阿里、蚂蚁、网易、字节杭州的 Agent/知识工程 JD，再定具体团队。', risk:'当前零条已核验杭州 JD，所有判断都还是空白，不能凭印象规划。' },
    { city:'深圳 / 广州 / 上海', role:'毕业后可接受', tier:'acceptable', strategy:'腾讯 CSIG（DataBuddy、Harness、元宝、PCG）集中在深圳，是当前证据最强的一组；上海可承接百度 ACG 方向。在杭州数据补齐前，深圳是最扎实的主线。', risk:'仍需确认各团队是否在该 base 开放学生岗；日常实习常要求每周 4—5 天、连续 5—6 个月。' },
    { city:'北京', role:'不作为投递目标', tier:'avoid', strategy:'按你的意向排除。北京岗位只保留“能力情报”价值：从其 JD 反推需要哪些能力与指标，用来指导作品集和面试表达，不投递。', risk:'样本中 84.2% 的记录集中在北京，因此“岗位多”不等于“机会多”——排除北京后，可投池显著变小，必须靠杭州补数据和深圳/成都主线来弥补。' }
  ],
  risks: {
    data: ['关键词样本不是公司招聘全集','百度与腾讯字段不可比：腾讯缺任职要求','城市可重复计数，记录数不等于 HC','updateDate 不能单独证明仍在招','关键词存在率不等于硬性要求率'],
    career: [
      '主动排除北京会显著缩小样本：百度技术岗集中在北京（1084/1287 条记录含北京），放弃该城市等于放弃当前证据最强的一批完整 JD',
