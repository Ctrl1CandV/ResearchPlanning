...
window.RESEARCH = {
  ...
  summary: [
    '论文主线在 2026-09 定稿：面向工具调用智能体的任务完成验证，核心是预算约束下的环境证据选择。Agent 宣称完成后，查什么、查多少、何时承认无法确认。',
    '方向只保留三条：主线 A，扩展 B 和 E；C/D/F 已收进档案（条件备选与暂缓），资料保留在各自阅读轨道里，不再占方向卡。',
    '三个最近工作先划界再干活：2606.09863 的轻量检测器是必比基线；Strategic Verification（预印本）要求逐项对表；2608.02645 说明读回与重试只是工程实践。',
    '最低实验原则：优先公开环境与真实轨迹；比较时同预算同接口权限；全量核验作成本参照；观察检出率、误报率、无法确认比例、成本和延迟；不靠大量弃权压误报。',
    '第一阶段的产出允许是否定式的：发现边界、收窄问题、否定方案都算数。研一争取完成主要实验和论文初稿；投稿与毕业要求分别核验。'
  ],
  readingSummary: [
    '公共必读 12 篇全部服务主线：前三篇立问题，中间六篇讲检测与证据获取，最后三篇是归因与评测方法。原先偏记忆的十篇移到了方向 C 和 D 的延伸层，走对应轨道时再读。',
    'Strategic Verification 是预印本，没有站内阅读卡，从外链读，读完先写划界笔记；其余每篇都有阅读卡，里面有导读和 arXiv 原文入口。',
    '方向卡只留三条：主线 A 先走；B 和 E 的预备阅读排在 90 天计划的 W8 和 W9；C/D/F 已收进档案，资料在各自轨道里，直接链接可达。',
    '90 天计划前 4 周做问题验证：划界、环境、证据点清单、综述初稿。W8 是最关键的检查点，划界站不住就当场调整，不硬撑。'
  ],
  ...
  plan90: [
    { w:'W1', ph:1, read:'公共必读第 1—3 篇：假成功刻画（2606.09863）、Strategic Verification（预印本，外链）、Verified Tool Calls（2608.02645）', run:'核实三篇的数据与代码可得性；写下与三个最近工作的差别一页纸：验证对象、预算定义、实验设置各占一行', out:'与最近工作的差别一页纸初稿；数据与代码可得性备忘', stuck:'预印本没有定稿章节的，按摘要划界并注明未读全文；差别写不出来的地方就是 W2 要补读的地方' },
